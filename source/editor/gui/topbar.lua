...
topbar.brush123 = gui:create("button", {text = "Brush", func = function() print("skibidi") editor.tool = "brush" editor.brush = 1 end})
topbar.erase123 = gui:create("button", {text = "Erase", func = function() editor.tool = "brush" editor.brush = 0 end, x = 100})
...
function topbarLoad(map)
    ...
    gui:style({button2, {font = 18, x = 10, y = 10, width = 80, height = 80}})
    topbar.brush = gui:create("button", {text = "Brush", func = function() editor.tool = "brush" editor.brush = 1 end})
    topbar.rectangle = gui:create("button", {text = "Box", func = function() end, x = 100})
    topbar.erase = gui:create("button", {text = "Erase", func = function() editor.tool = "brush" editor.brush = 0 end, x = 190})
    topbar.grid = gui:create("button", {text = "Show Gird", func = function() editor.grid = not editor.grid end, x = 300})

    gui:style({button2, {font = 18, x = 10, y = 10, width = 80, height = 80}})
    for i = 1, #map.layers do
        local layerbutton = gui:create("button", {text = "Layer "..i, x = 90 * (i + 4), func = function(self) editor.layer = self.number end, name = "layer button "..i})
        layerbutton.number = i
        function layerbutton:customUpdate(self, dt)
            if editor.layer == self.number then
                self.color = self.hoverColor
