...
create.next = gui:create("button", {text = "Next", x = 190, func = function() 
    local succes, result = pcall(function()
        editor:createMap({name = create.nameInput.text, directory = create.saveInput.text.."/"..create.nameInput.text..".lua", width = tonumber(create.dimensionsWidth.text), height = tonumber(create.dimensionsHeight.text), layers = tonumber(create.layersInput.text), scale = tonumber(create.scaleInput.text)})
    end)
