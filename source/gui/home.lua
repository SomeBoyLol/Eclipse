...
function resetRecent()
    gui:style({section})
    local recent = gui:create("frame", {text = "Open recent", x = 50, width = 850})
...
function createRecent(name, directory, date, num)
    num = num - 1
    gui:style({mediumbutton})
    local button = gui:create("button", {text = "", y = (110 * num) + 70, func = function(self)
        success, error = pcall(function() editor:openMap(self.directory.text) end)
        if success then
            menu = editorGUI
        else
            print("Couldn't open recent file: "..error)
        end
    end})
    button.dir = directory
    button.name = name
