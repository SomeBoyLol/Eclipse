...
newTileset.create = gui:create("button", {text = "Create", x = 190, func = function()
    local success, error = pcall(function() editor:loadTileset({sprite = love.graphics.newImage(newTileset.sourceInput.text), scale = tonumber(newTileset.scaleInput.text)}) end)
    print(error)
    if success then
        menu = editorGUI
    else
